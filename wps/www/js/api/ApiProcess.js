...
export class ApiProcess {
    ...
    static async getAllProcesses() {
        ...
        json.processes.forEach((value) => {
            listProc.push(new Process(value.description, value.id, value.inputs, value.outputs, value.links, value.title));
        });
    ...
    static async getSpecificProcess(processID) {
        ...
        return new Process(json.description, json.id, json.inputs, json.outputs, json.links, json.title);
    }
